...
func TestGetRequestID(t *testing.T) {
	// valid request ID
	ctxWithID := context.WithValue(context.Background(), requestIDKey, "id123")
	assert.Equal(t, "id123", getRequestID(ctxWithID))
	...
	ctxWrongType := context.WithValue(context.Background(), requestIDKey, 42)
	assert.Empty(t, getRequestID(ctxWrongType))
}
...
func TestCtxLogging_InjectsRequestID(t *testing.T) {
	...
	ctx := WithRequestID(context.Background(), "req-edge")
	CtxInfo(ctx, "info with reqid")

	log := buf.String()
	assert.Contains(t, log, `"request_id":"req-edge"`)
...
func TestCtxLogging_NoRequestID(t *testing.T) {
	...
	log := buf.String()
	assert.NotContains(t, log, `"request_id"`)
...
func TestCtxError_IncludesErrorAndRequestID(t *testing.T) {
	...
	ctx := WithRequestID(context.Background(), "req-error")

	CtxError(ctx, "error occurred", err)

	log := buf.String()
	assert.Contains(t, log, `"error":"fatal error"`)
	assert.Contains(t, log, `"request_id":"req-error"`)
...
func TestWithRequestID_RoundTrip(t *testing.T) {
	ctx := WithRequestID(context.Background(), "roundtrip-id")
	assert.Equal(t, "roundtrip-id", getRequestID(ctx))
}
...
func TestCtxDebug_WithAndWithoutRequestID(t *testing.T) {
	...
	out := buf.String()
	assert.Contains(t, out, `"request_id":"rid-debug"`)
...
func TestCtxWarn_WithAndWithoutRequestID(t *testing.T) {
	...
	t.Run("warn with request ID", func(t *testing.T) {
		buf.Reset()
		ctx := WithRequestID(context.Background(), "req-warn")
		CtxWarn(ctx, "warning with reqid")

		log := buf.String()
		assert.Contains(t, log, `"request_id":"req-warn"`)
